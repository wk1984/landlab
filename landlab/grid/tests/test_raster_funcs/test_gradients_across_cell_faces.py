...
def test_scalar_arg():
    grads = rfuncs.gradient_across_cell_face(
...
@with_setup(setup_unit_grid)
def test_iterable():
    grads = rmg.gradient_across_cell_face(values_at_nodes, [0, 4])
    assert_array_equal(grads, np.array([[1., 5., -1., -5.],
                                        [1., 5., -1., -5.]]))
...
def test_with_no_cell_id_arg():
    ...
    grads = rmg.gradient_across_cell_face(values)
...
def test_with_out_keyword():
    out = np.empty((1, 4))
    rtn = rmg.gradient_across_cell_face(values_at_nodes, 5, out=out)
    assert_is(rtn, out)
